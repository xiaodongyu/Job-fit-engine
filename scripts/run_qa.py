#!/usr/bin/env python3
"""
QA test runner per docs/qa-testing-plan.md.
Runs Phases 1–5, resolves paths from manifests, calls backend, evaluates rules, produces JSON + MD report.

Usage:
  python scripts/run_qa.py [--base-url URL] [--output-dir DIR] [--phases 1,2,3,4,5]
...
# Ensure scripts/ on path before importing qa_helpers
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Run QA phases per docs/qa-testing-plan.md")
    ap.add_argument("--base-url", default="http://localhost:8000", help="Backend base URL")
    ap.add_argument("--output-dir", "-o", default=".", help="Output directory for qa_report.json and qa_report.md")
